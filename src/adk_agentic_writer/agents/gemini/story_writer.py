"""Gemini-powered story writer agent.

Generates branched narratives using LLM-powered text generation.
...
class GeminiStoryWriterAgent(ContentWriterAgent):
    """Gemini story writer using LLM-powered generation.

    Same structure as StoryWriterAgent but uses
    GeminiTextProvider for natural language generation.
    """

    def __init__(self, agent_id: str = "gemini_story_writer"):
        """Initialize Gemini story writer.
        ...
        logger.info(f"Initialized GeminiStoryWriterAgent {agent_id}")

    async def _build_content(self, context: Dict[str, Any]) -> Dict[str, Any]:
        """Build branched narrative content using LLM.
        ...
        logger.info(
            f"Generating story with Gemini: {topic}, genre: {genre}, nodes: {num_nodes}"
        )
    ...
    async def _generate_story_nodes(
        ...
        """Generate story nodes with branches using LLM.
...
__all__ = ["GeminiStoryWriterAgent"]
