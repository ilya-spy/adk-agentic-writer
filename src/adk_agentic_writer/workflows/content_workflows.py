"""Content workflow patterns for user experience interaction.
...
from .base_workflow import ConditionalWorkflow, LoopWorkflow, SequentialWorkflow

logger = logging.getLogger(__name__)


class SequentialContentWorkflow(SequentialWorkflow):
    """
    ...
    def __init__(self, name: str, block_types: List[str], generator: Any):
        ...
        logger.info(f"Sequential content workflow '{name}' configured with {len(block_types)} blocks")


class LoopedContentWorkflow(LoopWorkflow):
    """
    ...
    def __init__(
        ...
        """
        super().__init__(name, generator, lambda data, iteration: not validator(data) and iteration < max_iterations, max_iterations)
        self.block_type = block_type
        self.validator = validator
        logger.info(f"Looped content workflow '{name}' configured for {block_type} with max {max_iterations} iterations")


class ConditionalContentWorkflow(ConditionalWorkflow):
    """
    ...
    def __init__(
        ...
        Args:
            ...
            default_generator: Default generator if condition returns unknown type
        """
        super().__init__(name, condition_fn, generators, default_generator)
        logger.info(f"Conditional content workflow '{name}' configured with {len(generators)} branches")


class InteractiveContentWorkflow(SequentialWorkflow):
    """
    ...
    def __init__(
        ...
        logger.info(f"Interactive content workflow '{name}' configured with max {max_interactions} interactions")


class AdaptiveContentWorkflow(SequentialWorkflow):
    """
    ...
    def __init__(
        ...
        """
        super().__init__(name, [default_generator])
        self.behavior_analyzer = behavior_analyzer
        self.generators = generators
        self.default_generator = default_generator
        logger.info(f"Adaptive content workflow '{name}' configured with {len(generators)} adaptation strategies")


class StreamingContentWorkflow(SequentialWorkflow):
    """
    ...
    def __init__(
        ...
        logger.info(f"Streaming content workflow '{name}' configured with buffer size {buffer_size}")
