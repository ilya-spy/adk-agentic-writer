...
async def test_all_formats():
    ...
    # Initialize coordinator (agents auto-registered via runtime)
    coordinator = CoordinatorAgent()

    # Test cases using generate_content
    tests = [
        {
            "name": "Quiz",
            "content_type": "quiz",
    ...
    for test in tests:
        ...
        try:
            result = await coordinator.generate_content(
                content_type=test["content_type"], topic=test["topic"], **test["params"]
            )
