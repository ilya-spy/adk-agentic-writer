...
def test_refinement_pipeline_structure():
    ...
    loop = create_refinement_pipeline(reviewer, refiner, max_iterations=3)

    assert isinstance(loop, LoopAgent)
    assert loop.max_iterations == 3
    assert len(loop.sub_agents) == 2
    assert loop.sub_agents[0].name == "ReviewerAgent"
...
def test_publish_pipeline_structure():
    ...
    assert inner_loop.sub_agents[0].name == "ReviewerAgent"
